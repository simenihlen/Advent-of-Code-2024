...
function validateMul(mulString: string): number{
    ...
    */
    ...
    while((match = controllReg.exec(mulString)) !== null) {
        //Collect the slice between lastIndex and match.index into chunk to 
        const chunk = mulString.slice(lastIndex, match.index)
        ...
        lastIndex = controllReg.lastIndex
    }

    const remainChunk = mulString.slice(lastIndex)
...
console.log("Total sum: ", validateMul(data))
//test output: 48
//data output: 